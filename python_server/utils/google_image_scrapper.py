...
class GoogleImageScraper():
        ...
        for indx in range (1,self.number_of_images+1):
            try:
                ...
                for image in images:
                    ...
                    if (image.get_attribute("src")[-3:].lower() in ["jpg", "jpeg"]):
                ...
                print(e, "GoogleImageScraper Skip: Unable to get the link for this photo")
    ...
    def save_images(self,image_urls):
        ...
        for indx,image_url in enumerate(image_urls):
            try:
                print(self.search_key.replace(" ", "_").lower())
                print(image_url.replace(" ", "_").replace("-", "_").lower())
                if self.search_key.replace(" ", "").lower() in image_url.replace("_", "").replace("-", "").lower():
                    #print("skipped")
                    filename = self.search_key+str(indx)+'_correct.jpg'
                else:
